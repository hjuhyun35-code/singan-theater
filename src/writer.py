...
SLIDE_TYPES = {
    "훅": (
        "표지와 함께 나가는 첫 장. 여기서 멈추지 않으면 나머지는 아무도 안 본다. "
        "★책 제목·저자·출판사를 headline 이나 body 에 쓰지 마라. "
        "  표지 그림에 이미 크게 박혀 있어서 글자로 또 쓰면 자리만 버린다. "
        "  이 자리는 오직 '멈추게 만드는 한마디' 를 위한 것이다. "
        "★headline 을 뭉뚱그리지 마라. 요약하지 말고 장면을 보여줘라. "
        "★body 를 반드시 채워라. 시간·장소·사물·숫자 중 최소 하나를 넣어 "
    ...
    "여운": (
        "마지막 장. ★추천하지 마라. 이게 핵심이다. "
        "'이런 사람에게', '읽어보자', '추천', '~라면 읽어라' 같은 말은 전부 금지다. "
        "그런 말투는 광고처럼 읽혀서 앞에서 쌓은 분위기를 무너뜨린다. "
        "대신 이 이야기가 남기는 인상 한 줄로 닫아라. 독자가 책을 덮고도 "
        "머리에 남을 법한 문장, 또는 앞 장면의 여운을 이어받는 문장. "
        "판단은 독자에게 맡기고 조용히 끝내라. kicker 는 비워두거나 아주 짧게."
    ),
}
